...
export async function POST(req: NextRequest) {
  try {
    ...
    const communityImageFile = data.get("communityImage") as File
    ...
    let communityImageUrl: string | undefined
    if (organizedBy === "community" && communityImageFile && communityImageFile.size > 0) {
      try {
        const { cid: communityCID } = await pinata.upload.public.file(communityImageFile)
        communityImageUrl = convertIPFSToHTTP(`ipfs://${communityCID}`)
      } catch (error) {
      ...
      speakers.map(async (speaker: any) => {
        ...
        if (speakerImageFile && speakerImageFile.size > 0) {
          try {
            ...
            const speakerImageUrl = convertIPFSToHTTP(`ipfs://${speakerImageCID}`)
            return {
              ...speaker,
